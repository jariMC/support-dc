# Datenschutzerklärung (Privacy Policy)

**Stand:** 8.4.2026

## 1. Allgemeine Hinweise

Diese Datenschutzerklärung informiert Sie über die Art, den Umfang und Zweck der Verarbeitung personenbezogener Daten auf dieser Website.

## 2. Verantwortlicher

Discord jarlet753

## 3. Erhebung und Speicherung personenbezogener Daten

Beim Besuch der Website werden automatisch Informationen durch den Webserver erfasst (z. B. IP-Adresse, Browsertyp, Uhrzeit des Zugriffs). Diese Daten dienen der technischen Bereitstellung und Sicherheit der Website.

## 4. Verwendung von Cookies

Diese Website verwendet Cookies. Cookies sind kleine Textdateien, die auf Ihrem Endgerät gespeichert werden.

## 5. Kontaktformular

Wenn Sie uns per Formular kontaktieren, werden Ihre Angaben zur Bearbeitung der Anfrage gespeichert.

## 6. Rechtsgrundlagen

Die Verarbeitung erfolgt gemäß Art. 6 Abs. 1 DSGVO.

## 7. Ihre Rechte

Sie haben das Recht auf Auskunft, Berichtigung, Löschung und Einschränkung der Verarbeitung Ihrer Daten.

## 8. Speicherdauer

Personenbezogene Daten werden nur so lange gespeichert, wie es für den jeweiligen Zweck erforderlich ist.

## 9. Sicherheit

Wir verwenden geeignete technische und organisatorische Maßnahmen zum Schutz Ihrer Daten.

---

# Privacy Policy (English)

**Last updated:** 4.8.2026

## 1. General Information

This Privacy Policy explains how we collect and process personal data.

## 2. Controller

Discord jarlet753

## 3. Data Collection

We automatically collect data such as IP address and browser type when you visit our website.

## 4. Cookies

We use cookies to improve user experience.

## 5. Contact Forms

Data submitted through forms is stored for processing inquiries.

## 6. Legal Basis

Processing is based on applicable laws (e.g., GDPR Art. 6).

## 7. User Rights

You have the right to access, correct, and delete your data.

## 8. Data Retention

We retain data only as long as necessary.

## 9. Security

We implement appropriate security measures.
